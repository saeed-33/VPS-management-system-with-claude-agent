...
يعتمد مباشرة على: app.capabilities.knowledge.ingestion_service، app.capabilities.knowledge.parsers، app.capabilities.knowledge.source_loader.
...
from app.capabilities.knowledge.ingestion_service import (
...
class SourceRepository:
    """
    يمثل SourceRepository جزءًا من طبقة Test suite.
    ...
    def __init__(self, source):
        ...
        تُستدعى عندما يصل المسار إلى __init__؛ المدخلات المهمة: source.
    ...
    def get_by_id(self, source_id):
        ...
        تُستدعى عندما يصل المسار إلى get_by_id؛ المدخلات المهمة: source_id.
        تعيد نتيجة العملية الحالية أو تسجل/ترجع الأثر الذي يحدده هذا الـworkflow. قد يعيد exit code أو يرفع exception عند فشل المدخلات أو dependency.
        """
...
class Loader:
    """
    يمثل Loader جزءًا من طبقة Test suite.
    ...
    def load(self, source):
        ...
        تُستدعى عندما يصل المسار إلى load؛ المدخلات المهمة: source.
        تعيد نتيجة العملية الحالية أو تسجل/ترجع الأثر الذي يحدده هذا الـworkflow. قد يعيد exit code أو يرفع exception عند فشل المدخلات أو dependency.
        """
...
class DocumentRepository:
    """
    يمثل DocumentRepository جزءًا من طبقة Test suite.
    ...
    def upsert_parsed(self, **kwargs):
        ...
        تُستدعى عندما يصل المسار إلى upsert_parsed؛ المدخلات المهمة: لا توجد مدخلات موضعية مهمة.
        تعيد نتيجة العملية الحالية أو تسجل/ترجع الأثر الذي يحدده هذا الـworkflow. قد يعيد exit code أو يرفع exception عند فشل المدخلات أو dependency.
        """
        self.saved = kwargs
    ...
    def mark_failed(self, **kwargs):
        ...
        تُستدعى عندما يصل المسار إلى mark_failed؛ المدخلات المهمة: لا توجد مدخلات موضعية مهمة.
        تعيد نتيجة العملية الحالية أو تسجل/ترجع الأثر الذي يحدده هذا الـworkflow. قد يعيد exit code أو يرفع exception عند فشل المدخلات أو dependency.
        """
        self.failed = kwargs
...
def test_ingestion_persists_parsed_document():
    ...
    تُستدعى عندما يصل المسار إلى test_ingestion_persists_parsed_document؛ المدخلات المهمة: لا توجد مدخلات موضعية مهمة.
