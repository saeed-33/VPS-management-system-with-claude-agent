"""Tests for test sandbox readiness.
...
def test_phase6_real_runtime_blocker_keeps_gate_closed():
    ...
    تُستدعى عندما يصل المسار إلى test_phase6_real_runtime_blocker_keeps_gate_closed؛ المدخلات المهمة: لا توجد مدخلات موضعية مهمة.
...
def test_phase6_gate_requires_all_thirteen_metrics():
    ...
    تُستدعى عندما يصل المسار إلى test_phase6_gate_requires_all_thirteen_metrics؛ المدخلات المهمة: لا توجد مدخلات موضعية مهمة.
    تعيد نتيجة العملية الحالية أو تسجل/ترجع الأثر الذي يحدده هذا الـworkflow. يفشل الاختبار عند خرق الـcontract.
    """
