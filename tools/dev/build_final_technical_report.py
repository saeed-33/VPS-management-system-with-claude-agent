...
def appendices(doc):
    ...
    for x in ["المصدر البرمجي: app/core، app/capabilities، app/runtime/claude، app/interfaces، app/infrastructure، app/composition.", "الاختبارات: tests/، مع استبعاد tests/acceptance/external_runtime من regression العادي.", "التحقق: tools/bootstrap_database.py، tools/dev/list_routes.py، ProjectMcpToolBoundary.list_tools().", "الوثائق المرجعية: دفتر الشروط PDF، القالب 1-Report-Template.docx، والتقرير العربي القديم كمصدر مصطلحات فقط.", "الهوية الطلابية الموثقة في هذه النسخة: سعيد بقدونس."]:
        add_bullet(doc, x)
