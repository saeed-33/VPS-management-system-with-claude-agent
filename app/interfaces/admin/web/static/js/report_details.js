/**
 * صفحة تفاصيل Monitoring Report. تقرأ report وanalysis وretrieval sources من Admin API، ثم تبني العرض؛ لا تنفذ Analysis أو Retrieval بنفسها.
 */
const reportPageData = document.getElementById(
    "report-page-data"
);

const reportId = Number(
    reportPageData.dataset.reportId
);
/**
 * ينفذ خطوة واجهة باسم loadReportDetails ضمن صفحة Admin Web.
 * يقرأ state من DOM أو API ويحدث العرض؛ الفشل يظهر للمستخدم أو يمرر للـcaller.
 */
async function loadReportDetails() {
    try {
        const report = await apiRequest(
            `/api/reports/${reportId}`
        );

        document.getElementById(
            "report-page-description"
        ).textContent =
            `${report.server_name} — ` +
            `${formatDate(report.started_at)}`;

        document.getElementById(
            "report-status-value"
        ).innerHTML =
            statusBadge(report.status);

        document.getElementById(
            "report-success-count"
        ).textContent =
            report.commands_succeeded;

        document.getElementById(
            "report-failed-count"
        ).textContent =
            report.commands_failed;

        document.getElementById(
            "report-duration"
        ).textContent =
            formatDuration(report.duration_ms);

        renderReportInformation(report);
        renderCommandExecutions(
            report.executions
        );

        await loadReportAnalysis();
        await loadAnalysisSources();

    } catch (error) {
        showToast(
            error.message,
            "error"
        );
    }
}

/**
 * ينفذ خطوة واجهة باسم loadReportAnalysis ضمن صفحة Admin Web.
 * يقرأ state من DOM أو API ويحدث العرض؛ الفشل يظهر للمستخدم أو يمرر للـcaller.
 */
async function loadReportAnalysis() {
    const container =
        document.getElementById(
            "report-analysis"
        );

    try {
        const analysis = await apiRequest(
            `/api/reports/${reportId}/analysis`
        );

        renderReportAnalysis(analysis);

    } catch (error) {
        container.innerHTML = `
            <div class="empty-state">
                <h3>التحليل غير متوفر</h3>

                <p>
                    قد يكون التحليل قيد التنفيذ،
                    أو أن مزود LLM غير متاح.
                </p>
            </div>
        `;
    }
}

/**
 * ينفذ خطوة واجهة باسم loadAnalysisSources ضمن صفحة Admin Web.
 * يقرأ state من DOM أو API ويحدث العرض؛ الفشل يظهر للمستخدم أو يمرر للـcaller.
 */
async function loadAnalysisSources() {
    const container =
        document.getElementById(
            "analysis-sources"
        );

    try {
        const response = await apiRequest(
            `/api/reports/${reportId}/analysis-sources`
        );

        renderAnalysisSources(
            response.sources || []
        );

    } catch (error) {
        container.innerHTML = `
            <div class="empty-state">
                <h3>لا توجد مصادر مسجلة</h3>
                <p>
                    قد يكون التحليل غير مكتمل أو أُنشئ
                    قبل إضافة تتبع المصادر.
                </p>
            </div>
        `;
    }
}


/**
 * ينفذ خطوة واجهة باسم renderAnalysisSources ضمن صفحة Admin Web.
 * يقرأ state من DOM أو API ويحدث العرض؛ الفشل يظهر للمستخدم أو يمرر للـcaller.
 */
function renderAnalysisSources(sources) {
    const container =
        document.getElementById(
            "analysis-sources"
        );

    if (!sources.length) {
        container.innerHTML = `
            <div class="empty-state">
                <h3>لا توجد مصادر</h3>
            </div>
        `;
        return;
    }

    container.innerHTML = `
        <div class="analysis-sources-list">
            ${sources.map(source => `
                <article class="analysis-source-card">
                    <header>
                        <strong>
                            ${escapeHtml(source.title)}
                        </strong>

                        <span class="badge ${
                            source.used_in_prompt
                                ? "badge-enabled"
                                : "badge-unknown"
                        }">
                            ${
                                source.used_in_prompt
                                    ? "استُخدم داخل سياق LLM"
                                    : "مرجع فقط"
                            }
                        </span>
                    </header>

                    <div class="analysis-summary">
                        <div>
                            <span class="form-label">
                                نوع المصدر
                            </span>
                            <strong>
                                ${escapeHtml(
                                    source.source_type
                                )}
                            </strong>
                        </div>

                        <div>
                            <span class="form-label">
                                رقم التقرير
                            </span>
                            <strong>
                                ${
                                    source.source_report_id
                                        ? `#${source.source_report_id}`
                                        : "-"
                                }
                            </strong>
                        </div>

                        <div>
                            <span class="form-label">
                                رقم التحليل
                            </span>
                            <strong>
                                ${
                                    source.source_analysis_id
                                        ? `#${source.source_analysis_id}`
                                        : "-"
                                }
                            </strong>
                        </div>

                        <div>
                            <span class="form-label">
                                طريقة الاسترجاع
                            </span>
                            <strong>
                                ${formatRetrievalStrategy(
                                    source.retrieval_strategy
                                )}
                            </strong>
                        </div>

                        <div>
                            <span class="form-label">
                                درجة التشابه
                            </span>
                            <strong>
                                ${
                                    source.similarity_score !== null
                                    && source.similarity_score !== undefined
                                        ? `${Math.round(
                                            source.similarity_score * 100
                                        )}%`
                                        : "-"
                                }
                            </strong>
                        </div>
                    </div>

                    ${
                        source.excerpt
                            ? `
                                <div class="analysis-source-excerpt">
                                    <code>${escapeHtml(
                                        source.excerpt
                                    )}</code>
                                </div>
                            `
                            : ""
                    }
                </article>
            `).join("")}
        </div>
    `;
}

/**
 * ينفذ خطوة واجهة باسم renderReportAnalysis ضمن صفحة Admin Web.
 * يقرأ state من DOM أو API ويحدث العرض؛ الفشل يظهر للمستخدم أو يمرر للـcaller.
 */
function renderReportAnalysis(analysis) {
    const container =
        document.getElementById(
            "report-analysis"
        );

    if (analysis.status === "pending") {
        container.innerHTML = `
            <div class="analysis-state">
                <span class="loading-spinner"></span>

                <div>
                    <strong>
                        التحليل في قائمة الانتظار
                    </strong>

                    <p>
                        سيقوم وكيل السيرفر بتحليل
                        التقرير قريبًا.
                    </p>
                </div>
            </div>
        `;

        return;
    }

    if (analysis.status === "running") {
        container.innerHTML = `
            <div class="analysis-state">
                <span class="loading-spinner"></span>

                <div>
                    <strong>
                        جارٍ تحليل التقرير
                    </strong>

                    <p>
                        النموذج:
                        ${escapeHtml(analysis.model_name)}
                    </p>
                </div>
            </div>
        `;

        return;
    }

    if (analysis.status === "failed") {
        container.innerHTML = `
            <div class="analysis-error-card">
                <strong>
                    فشل تحليل التقرير
                </strong>

                <p>
                    ${escapeHtml(
                        analysis.analysis_error ||
                        "خطأ غير معروف"
                    )}
                </p>
            </div>
        `;

        return;
    }

    const analysisSourceBadge =
        analysis.llm_called
            ? `
                <span class="badge badge-enabled">
                    تحليل جديد بواسطة LLM
                </span>
            `
            : `
                <span class="badge badge-reused">
                    تحليل تاريخي معاد الاستخدام
                </span>
            `;

    const allIssues = analysis.issues || [];
    const issues = allIssues.filter(
        issue =>
            issue.severity === "warning"
            || issue.severity === "critical"
    );

    const informationalFindings = allIssues
        .filter(issue => issue.severity === "info")
        .map(issue => issue.description || issue.title);

    const positiveFindings = [
        ...(analysis.positive_findings || []),
        ...informationalFindings
    ];

    container.innerHTML = `
        <div class="analysis-source-banner ${
            analysis.llm_called ? "generated" : "reused"
        }">
            <div>
                ${analysisSourceBadge}

                <strong>
                    ${
                        analysis.llm_called
                            ? "تم تحليل هذا التقرير بواسطة نموذج الذكاء الاصطناعي."
                            : "لم يتم استدعاء نموذج الذكاء الاصطناعي لهذا التقرير."
                    }
                </strong>

                <p>
                    ${
                        analysis.llm_called
                            ? `
                                النموذج المستخدم:
                                ${escapeHtml(analysis.model_name)}
                            `
                            : `
                                تم نسخ نتيجة تحليل سابق مطابق
                                باستخدام بصمة التقرير.
                            `
                    }
                </p>
            </div>

            ${
                analysis.reused_from_analysis_id
                    ? `
                        <div class="analysis-source-reference">
                            <span>التحليل المصدر</span>

                            <strong>
                                #${analysis.reused_from_analysis_id}
                            </strong>
                        </div>
                    `
                    : ""
            }
        </div>

        <div class="analysis-summary">
            <div>
                <span class="form-label">
                    الحالة العامة
                </span>

                <div>
                    ${analysisHealthBadge(
                        analysis.health_status
                    )}
                </div>
            </div>

            <div>
                <span class="form-label">
                    مصدر التحليل
                </span>

                <strong>
                    ${
                        analysis.analysis_source === "reused"
                            ? "تحليل سابق"
                            : "تحليل جديد"
                    }
                </strong>
            </div>

            <div>
                <span class="form-label">
                    استدعاء LLM
                </span>

                <strong>
                    ${
                        analysis.llm_called
                            ? "تم الاستدعاء"
                            : "لم يتم الاستدعاء"
                    }
                </strong>
            </div>

            <div>
                <span class="form-label">
                    طريقة الاسترجاع
                </span>

                <strong>
                    ${formatRetrievalStrategy(
                        analysis.retrieval_strategy
                    )}
                </strong>
            </div>

            <div>
                <span class="form-label">
                    درجة التطابق
                </span>

                <strong>
                    ${
                        analysis.retrieval_score !== null
                        && analysis.retrieval_score !== undefined
                            ? `${Math.round(
                                analysis.retrieval_score * 100
                            )}%`
                            : "-"
                    }
                </strong>
            </div>

            <div>
                <span class="form-label">
                    النموذج
                </span>

                <strong>
                    ${escapeHtml(analysis.model_name)}
                </strong>
            </div>
        </div>

        <div class="analysis-section">
            <h3>الملخص</h3>

            <p>
                ${escapeHtml(
                    analysis.summary ||
                    "لا يوجد ملخص"
                )}
            </p>
        </div>

        <div class="analysis-section">
            <h3>المشكلات المكتشفة</h3>

            ${
                issues.length
                    ? issues.map(issue => `
                        <article class="analysis-issue ${
                            issue.severity
                        }">
                            <header>
                                <strong>
                                    ${escapeHtml(
                                        issue.title
                                    )}
                                </strong>

                                <span class="badge ${
                                    issue.severity ===
                                    "critical"
                                        ? "badge-failed"
                                        : issue.severity ===
                                          "warning"
                                            ? "badge-partial_failure"
                                            : "badge-enabled"
                                }">
                                    ${escapeHtml(
                                        issue.severity
                                    )}
                                </span>
                                <span class="badge badge-unknown">
                                    ${escapeHtml(
                                        issue.classification || "unclassified"
                                    )}
                                </span>
                            </header>

                            <p>
                                ${escapeHtml(
                                    issue.description
                                )}
                            </p>

                            ${
                                issue.evidence
                                    ? `
                                        <div class="analysis-evidence">
                                            <strong>
                                                الدليل
                                            </strong>

                                            <pre>${escapeHtml(
                                                issue.evidence
                                            )}</pre>
                                        </div>
                                    `
                                    : ""
                            }

                            ${
                                issue.recommendation
                                    ? `
                                        <div class="analysis-recommendation">
                                            <strong>
                                                التوصية
                                            </strong>

                                            <p>
                                                ${escapeHtml(
                                                    issue.recommendation
                                                )}
                                            </p>
                                        </div>
                                    `
                                    : ""
                            }
                        </article>
                    `).join("")
                    : `
                        <p class="text-muted">
                            لم يكتشف النموذج مشكلات.
                        </p>
                    `
            }
        </div>

        <div class="analysis-lists">
            <section>
                <h3>النقاط الإيجابية</h3>

                ${renderAnalysisList(
                    positiveFindings
                )}
            </section>

            <section>
                <h3>الإجراءات المقترحة</h3>

                ${renderAnalysisList(
                    analysis.recommended_actions
                )}
            </section>
        </div>
    `;
}

/**
 * ينفذ خطوة واجهة باسم renderAnalysisList ضمن صفحة Admin Web.
 * يقرأ state من DOM أو API ويحدث العرض؛ الفشل يظهر للمستخدم أو يمرر للـcaller.
 */
function renderAnalysisList(items) {
    if (!items || !items.length) {
        return `
            <p class="text-muted">
                لا توجد عناصر.
            </p>
        `;
    }

    return `
        <ul class="analysis-list">
            ${items.map(item => `
                <li>
                    ${escapeHtml(item)}
                </li>
            `).join("")}
        </ul>
    `;
}

/**
 * ينفذ خطوة واجهة باسم formatRetrievalStrategy ضمن صفحة Admin Web.
 * يقرأ state من DOM أو API ويحدث العرض؛ الفشل يظهر للمستخدم أو يمرر للـcaller.
 */
function formatRetrievalStrategy(strategy) {
    const labels = {
        exact_fingerprint: "تطابق البصمة",
        structured: "بحث منظّم",
        full_text: "بحث نصي",
        vector: "بحث متجهي",
        hybrid: "بحث هجين"
    };

    return strategy
        ? labels[strategy] || strategy
        : "لا يوجد";
}

/**
 * ينفذ خطوة واجهة باسم analysisHealthBadge ضمن صفحة Admin Web.
 * يقرأ state من DOM أو API ويحدث العرض؛ الفشل يظهر للمستخدم أو يمرر للـcaller.
 */
function analysisHealthBadge(status) {
    const values = {
        healthy: {
            label: "سليم",
            className: "badge-success"
        },
        warning: {
            label: "تحذير",
            className: "badge-partial_failure"
        },
        critical: {
            label: "حرج",
            className: "badge-failed"
        },
        unknown: {
            label: "غير معروف",
            className: "badge-unknown"
        }
    };

    const value =
        values[status] ||
        values.unknown;

    return `
        <span class="badge ${value.className}">
            ${value.label}
        </span>
    `;
}

/**
 * ينفذ خطوة واجهة باسم renderReportInformation ضمن صفحة Admin Web.
 * يقرأ state من DOM أو API ويحدث العرض؛ الفشل يظهر للمستخدم أو يمرر للـcaller.
 */
function renderReportInformation(report) {
    const container =
        document.getElementById(
            "report-information"
        );

    container.innerHTML = `
        <div class="report-info-item">
            <span>رقم التقرير</span>
            <strong>#${report.id}</strong>
        </div>

        <div class="report-info-item">
            <span>السيرفر</span>
            <strong>
                ${escapeHtml(report.server_name)}
            </strong>
        </div>

        <div class="report-info-item">
            <span>العنوان</span>
            <strong dir="ltr">
                ${escapeHtml(report.server_host)}
            </strong>
        </div>

        <div class="report-info-item">
            <span>وقت البداية</span>
            <strong>
                ${formatDate(report.started_at)}
            </strong>
        </div>

        <div class="report-info-item">
            <span>وقت النهاية</span>
            <strong>
                ${formatDate(report.finished_at)}
            </strong>
        </div>

        <div class="report-info-item">
            <span>نجاح اتصال SSH</span>
            <strong>
                ${
                    report.connection_successful
                        ? "نعم"
                        : "لا"
                }
            </strong>
        </div>

        <div class="report-info-item">
            <span>إجمالي التعليمات</span>
            <strong>
                ${report.commands_total}
            </strong>
        </div>

        <div class="report-info-item">
            <span>رسالة الخطأ</span>
            <strong>
                ${
                    escapeHtml(
                        report.error_message ||
                        "لا يوجد"
                    )
                }
            </strong>
        </div>
    `;
}

/**
 * ينفذ خطوة واجهة باسم renderCommandExecutions ضمن صفحة Admin Web.
 * يقرأ state من DOM أو API ويحدث العرض؛ الفشل يظهر للمستخدم أو يمرر للـcaller.
 */
function renderCommandExecutions(executions) {
    const container =
        document.getElementById(
            "command-executions"
        );

    if (!executions.length) {
        container.innerHTML = `
            <div class="empty-state">
                <h3>لا توجد نتائج تعليمات</h3>

                <p>
                    لم ينفذ هذا التقرير أي تعليمات.
                </p>
            </div>
        `;

        return;
    }

    container.innerHTML = executions
        .sort(
            (first, second) =>
                first.execution_order -
                second.execution_order
        )
        .map(execution => `
            <article class="execution-card">
                <header class="execution-card-header">
                    <div>
                        <span class="execution-order">
                            ${execution.execution_order}
                        </span>

                        <div>
                            <h3>
                                ${escapeHtml(
                                    execution.command_name
                                )}
                            </h3>

                            <code class="execution-command">
                                ${escapeHtml(
                                    execution.command_text
                                )}
                            </code>
                        </div>
                    </div>

                    <div class="execution-status">
                        ${
                            execution.success
                                ? statusBadge("success")
                                : statusBadge("failed")
                        }
                    </div>
                </header>

                <div class="execution-meta">
                    <span>
                        Exit status:
                        ${execution.exit_status ?? "-"}
                    </span>

                    <span>
                        المدة:
                        ${formatDuration(
                            execution.duration_ms
                        )}
                    </span>

                    <span>
                        البداية:
                        ${formatDate(
                            execution.started_at
                        )}
                    </span>
                </div>

                ${
                    execution.error_message
                        ? `
                            <div class="execution-error">
                                ${escapeHtml(
                                    execution.error_message
                                )}
                            </div>
                        `
                        : ""
                }

                <div class="execution-output-grid">
                    <section>
                        <h4>stdout</h4>

                        <pre>${escapeHtml(
                            execution.stdout ||
                            "لا توجد مخرجات"
                        )}</pre>
                    </section>

                    <section>
                        <h4>stderr</h4>

                        <pre>${escapeHtml(
                            execution.stderr ||
                            "لا توجد أخطاء"
                        )}</pre>
                    </section>
                </div>
            </article>
        `)
        .join("");
}

loadReportDetails();
